...
<body>
    ...
    echo 'selected'; ?>>First Name</option>
                    <option value="sex"<?php if(isset($_GET['sort_field']) && $_GET['sort_field'] ==  'sex') 
    echo 'selected'; ?>>Sex</option>
                    <option value="bdate"<?php if(isset($_GET['sort_field']) && $_GET['sort_field'] ==  'bdate') 
    echo 'selected'; ?>>Birth Date</option>
...
$sql = "SELECT $select_clause FROM classmate_details WHERE lastname LIKE '%$filter_lname%' ORDER BY $sort_field $sort_order";
...
if ($result && $result->num_rows > 0) {
       while ($row = $result->fetch_assoc()) {
        ...
        echo "<td>
                                <a href='update.php?stud_no=".$row['stud_no']."'>Edit</a> |
                                <a href='delete.php?stud_no=".$row['stud_no']."' onclick=\"return confirm('Are you sure you want to delete this record?')\">Delete</a>
                               </td>";
        echo "</tr>";
    }
}
